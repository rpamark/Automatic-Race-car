...
def preprocess_frame(frame):
    ...
    green_channel = frame[:, :, 1] # shape: (height, width)
    resized = cv2.resize(green_channel, (84, 84), interpolation=cv2.INTER_AREA)
    # 归一化到 [0, 1]
...
class DQNAgent:
    def __init__(self, action_size, seed=42):
        ...
        self.buffer_size = 75000  # 减少经验回放缓冲区大小以节省显存
        self.batch_size = 32      # 使用较小的batch size (32是常用且对8GB显存友好的值)
        self.gamma = 0.99
        self.tau = 1e-3
        self.lr = 5e-4            # 稍微降低学习率可能有助于稳定
        self.update_every = 4
        self.epsilon = 0.8
        self.epsilon_min = 0.01
        self.epsilon_decay = 0.96 # 稍微减慢epsilon衰减，增加探索时间
...
# --- 6. 训练函数 (调整训练参数) ---
def train_agent(n_episodes=150, max_t=1000, solve_score=850.0):
    ...
    best_average_score = -np.inf  # 初始化最佳平均分数
    ...
    for i_episode in range(1, n_episodes+1):
        ...
        if current_average_score > best_average_score:
            ...
            print(f'\nNew best model saved with average score: {best_average_score:.2f}')
        # 打印进度，包括原始奖励
...
def plot_training_results(scores):
    ...
    if len(scores) >= window_size:
        ...
        plt.title(f'滑动平均得分 (窗口={window_size})')
        plt.xlabel('Episode')
...
def main():
    ...
    print("=" * 60)
    print(f"硬件配置: RTX 4060 8GB + 32GB RAM")
    print("优化策略:")
    print("- 使用4帧堆叠提供运动信息")
    ...
    scores, trained_agent = train_agent(n_episodes=150, max_t=1000, solve_score=800.0) 
